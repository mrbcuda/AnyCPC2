...
g_absoberShapes = {'Pentagon','MiniChannel','SimpleCircle','CircleWithGapConsideration','58mm absorber icecream cone design','85mm absorber icecream cone design','hand drawn 1','hand drawn 2','Octagon','random points','changed shape','square','flat','improvedPentagon', 'oval shape','v groove','hexagon'};
global g_absorber;
g_absorber = [];
...
for i=size(g_shapeData,2):-1:1
        pointStr = {sprintf('%.2f',g_shapeData(1,i)), sprintf('%.2f',g_shapeData(2,i))};
    ltData = [ltData;pointStr];
end
for i=1:size(g_shapeData,2)
        pointStr = {sprintf('%.2f',-g_shapeData(1,i)), sprintf('%.2f',g_shapeData(2,i))};
    ltData = [ltData;pointStr];
end
xlswrite('ltData.xls',ltData);
%end
